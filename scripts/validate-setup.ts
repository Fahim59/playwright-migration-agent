#!/usr/bin/env ts-node
/**
 * Validation System - Comprehensive Setup Checker
 * Validates all configuration files, JSON syntax, TypeScript compilation,
 * and project structure before running migrations
 */
...
function checkJsonValidity(filePath: string, name: string): ValidationResult {
  try {
    const content = fs.readFileSync(filePath, 'utf-8');
    
    // Check for JavaScript comments in JSON (common mistake)
...
function checkFileExists(filePath: string, name: string): ValidationResult {
  try {
...
function checkESModuleCompatibility(): ValidationResult {
  try {
    ...
    if (isESModule && usesCommonJS) {
      return {
        ...
        status: 'FAIL',
        message: 'package.json has "type": "module" but jest.config.js uses CommonJS (module.exports)',
...
function checkNpmScripts(): ValidationResult {
  try {
    const packageJsonPath = path.join(process.cwd(), 'package.json');
    const content = fs.readFileSync(packageJsonPath, 'utf-8');
    const packageJson = JSON.parse(content);
...
function checkTypeScriptConfig(): ValidationResult {
  try {
...
function checkNodeModules(): ValidationResult {
  try {
